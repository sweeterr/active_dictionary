...
def clean_text(path):
    with open(path, 'r', encoding='utf-8') as f:
        ...
        for line in lines[3:]:
            ...
            line = re.sub(' ', ' ', line)                           # sub nbrspace with regular space
            line = re.sub('([AА][0-9]+) % ', '\g<1>\t•\t', line)    # arg model with tabs
            line = re.sub('% ', '\t•\t', line)                      # arg model with tabs
            line = re.sub(' +', ' ', line)                          # delete multiple spaces
        ...
        new_text = re.sub('\t+', '\t', new_text)                                    # del multiple tabs
        new_text = re.sub(' +\t', '\t', new_text)                                   # del spaces before tabs
    return new_text
